...
其中 "collision 后单个 spur 的真实幅度" 要特别说明：当多个原始频率（例如 HD3 和
某个 clock spur）折到同一个 debug bin 时，它们在该 bin 上是**复数相量相加**，不是
功率相加。所以这个 bin 的幅度可能升高、降低，甚至相互抵消——单看这一个 bin 的幅度，
既无法判断里面有几个 spur，也无法还原任何一个 spur 的真实幅度。这正是为什么需要
multi-N / multi-fin 来分开候选（见后面的正确用法）。
